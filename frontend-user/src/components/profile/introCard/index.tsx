    ...
    <div className=" static space-y-3 lg:sticky  lg:-top-44 ">
      ...
      <div className="">
        ...
        >
          ...
          {getSingleUser?.data?.data?._id === currentUser?.data?._id ? (
            <div className="my-5 grid grid-cols-2 gap-4">
              {crux?.crux
                .filter(
                  (fil: any) =>
                    fil?.level <=
                      Number(getSingleUser?.data?.data?.cruxLevel) + 2 &&
                    fil?.level >=
                      Number(getSingleUser?.data?.data?.cruxLevel) - 1
                )
                ?.map((item: any) => (
                  <div
                    key={item?.label}
                    className={`flex justify-center rounded-md border ${
                      item?.level > getSingleUser?.data?.data?.cruxLevel
                        ? "bg-gray-100"
                        : "bg-white"
                    } px-4 py-2`}
                  >
                      ...
                      <div
                        className={` ${
                          item?.level > getSingleUser?.data?.data?.cruxLevel
                            ? ""
                            : "text-[#4bb543]"
                        } text-center text-lg font-[600] text-primary-400`}
                      >
                        {item?.label}
                      </div>
            ...
            <div>
              {otherUserCrux ? (
                <div
                  key={otherUserCrux?.title}
                  className={`flex justify-center rounded-md border ${"bg-white"} px-4 py-2`}
                >
                  <div className="">
                    <div className="flex justify-center">
                      <UnlockIcon />
                    </div>
                    <div className="text-center text-lg font-[600] text-primary-400">
                      {otherUserCrux?.title}
                    </div>
                    {/* <div className="text-center text-[15px] text-gray-600">
                    {otherUserCrux?.des}
                ...
                <div className="py-10 text-center text-xl font-medium text-primary-400">
                  No crux achieved yet!{" "}
                </div>
              )}
            </div>
          )}
        ...
        <SuggestFollowList
          getSuggestedFollowersList={getSuggestedFollowersList}
